        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 120px', background: '#F6F1E8' }}>
          <Routes>
            ...
            <Route path="hero" element={
              <HeroAdmin data={draft[lang].hero} onChange={v => updateSection('hero', v)} />
            } />
            <Route path="about" element={
              <AboutAdmin
                data={draft[lang].about}
                onChange={v => updateSection('about', v)}
                aneliaData={draft[lang].anelia}
                onAneliaChange={v => updateSection('anelia', v)}
                philosophyData={draft[lang].philosophy}
                onPhilosophyChange={v => updateSection('philosophy', v)}
                highlightsData={draft[lang].highlights}
                onHighlightsChange={v => updateSection('highlights', v)}
              />
            } />
            <Route path="consulting" element={
              <ConsultingAdmin data={draft[lang].consulting} onChange={v => updateSection('consulting', v)} />
            } />
            <Route path="training" element={
              <TrainingAdmin data={draft[lang].training} onChange={v => updateSection('training', v)} />
            } />
            <Route path="testimonials" element={
              <TestimonialsAdmin data={draft[lang].testimonials} onChange={v => updateSection('testimonials', v)} />
            } />
            <Route path="media" element={
              <MediaAdmin data={draft[lang].media} onChange={v => updateSection('media', v)} />
            } />
            <Route path="contact" element={
              <ContactAdmin data={draft[lang].contact} onChange={v => updateSection('contact', v)} />
            } />
            <Route path="nav-footer" element={
              <NavFooterAdmin
                navData={draft[lang].nav}
                onNavChange={v => updateSection('nav', v)}
                footerData={draft[lang].footer}
                onFooterChange={v => updateSection('footer', v)}
              />
            } />
          </Routes>
        </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: saving ? '#A8C8D8' : '#4D8FAA',
              color: '#fff',
              border: 'none',
              transition: 'all 0.2s',
              minWidth: '120px',
